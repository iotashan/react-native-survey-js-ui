...
const checks = [
  {
    ...
    description: 'Validates library source code with strict TypeScript checking'
  },
  {
  ...
  },
  {
    ...
    command: 'tsc --noEmit --moduleResolution node --esModuleInterop --allowSyntheticDefaultImports lib/typescript/module/index.d.ts',
...
for (const check of checks) {
  ...
  try {
    ...
    results.push({ name: check.name, status: 'FAIL', error: error.stdout || error.stderr });
    allPassed = false;
  }
}
...
if (allPassed) {
  ...
  console.log(chalk.yellow('Please fix the TypeScript errors above and try again.'));
  process.exit(1);
}